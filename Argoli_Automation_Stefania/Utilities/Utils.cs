...

{
    public class Utils
    {
        ...
        public static string Encrypt(string source, string key)// primeste un string pe care aplica o cheie de criptare
        {
            using (TripleDESCryptoServiceProvider tripleDESCryptoService = new TripleDESCryptoServiceProvider())//module interne de criptografie
            {
                using (MD5CryptoServiceProvider hashMD5Provider = new MD5CryptoServiceProvider())
                {
                    ...
                    return Convert.ToBase64String(tripleDESCryptoService.CreateEncryptor().TransformFinalBlock(data, 0, data.Length));// returneaza un base64(metoda de reprezentare a datelor- un format in care pot sa codez un fisier binar)
                }
            }
        }
